/*
  ...
  I use `json.pegjs` as the base grammar, and add `regexp` variable to the production `value = ...`.
  And then, the production `regexp = ...` is just `RegularExpressionLiteral` variable in `javascript.pegjs`.
  I copy all related variables and terminals from `javascript.pegjs`.
*/
...
regexp = RegularExpressionLiteral    // use RegularExpressionLiteral
